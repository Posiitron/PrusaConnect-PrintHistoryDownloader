...
class PrusaDataFetcher {
  ...
  async initialize() {
    const tabs = await browser.tabs.query({ active: true, currentWindow: true }); // Use 'browser' instead of 'chrome'
  ...
  async downloadZipFile(blob) {
    try {
      ...
      // Use browser.downloads.download to initiate the download
      ...
      if (downloadId) {
        ...
        // Wait for the download to complete and then revoke the URL
        setTimeout(() => {
          URL.revokeObjectURL(url);
          console.log("URL revoked after download");
        }, 5000); // Wait 5 seconds before revoking, adjust if needed
